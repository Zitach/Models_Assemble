...
当前已实现 3 个 crate 的 workspace 结构，normalized schema 和 adapter trait 已稳定：
...
当前架构已完全实现 normalized protocol 层，所有 provider 通过 `ProviderAdapter` trait 统一接入。后续如需进一步拆分，可以考虑：
...
当前实现：步骤 1-9 已完全实现。步骤 10（usage、latency 记录）已部分实现（日志输出），但尚未持久化。

### 4.2 内部标准模型（已实现）

内部不要直接使用 OpenAI 或 Anthropic 的结构作为唯一真相。已定义自己的 normalized schema。

核心结构（已在 `crates/ma-core/src/normalized.rs` 中实现）：
...
实现状态：所有核心结构已实现，包含 23 个单元测试。

### 4.3 Streaming 与 tool use 状态机（已实现）

只定义 `NormalizedEvent` 还不够。实际实现必须把流式协议当成状态机处理，否则多 tool call、参数分片、content block 顺序和半截 JSON 都容易出错。

当前实现：OpenAI 和 Anthropic adapter 都实现了完整的流式状态机，支持多 tool call、参数分片和 content block 顺序管理。
...
- stream 开始前可以 fallback（first-chunk buffering）。
...
当前已实现（在 `crates/ma-core/src/adapter.rs` 中）：
...
- `OpenAiAdapter`（`crates/ma-server/src/adapters/openai.rs`，857 行）— 处理 OpenAI-compatible 协议。
- `AnthropicAdapter`（`crates/ma-server/src/adapters/anthropic.rs`，887 行）— 处理 Anthropic-compatible 协议。
- `MockAdapter`（`crates/ma-server/src/adapters/mock.rs`）— 用于测试和兼容性验证。

后续计划：

- Zhipu GLM Coding Plan adapter（当前通过 `anthropic_compatible` 类型接入）。
- Kimi adapter（当前通过 `anthropic_compatible` 类型接入）。
- DeepSeek adapter（当前通过 `openai_compatible` 和 `anthropic_compatible` 类型接入）。
- OpenRouter adapter。
...
状态：所有目标已完成。`ma-cli` 支持 `serve`、`compat-probe`、`validate`、`doctor`、`test-provider` 命令。

### Milestone 1: OpenAI-compatible 入口和出口 ✅ 已完成
...
状态：所有目标已完成。通过 `OpenAiAdapter` 实现 OpenAI-compatible provider 接入，支持非流式和 SSE 流式响应。

### Milestone 2: 模型别名和 fallback ✅ 已完成
...
状态：所有目标已完成。支持配置 fallback 链，非流式请求在可重试错误时自动切换 provider。流式请求支持 first-chunk fallback（在 `first_token_timeout_secs` 内未收到上游数据时切换）。

### Milestone 3: Anthropic-compatible 入口 ✅ 已完成
...
状态：所有目标已完成。通过 `AnthropicAdapter` 实现 Anthropic-compatible provider 接入，支持非流式和 SSE 流式响应。包含完整的 tool use 跨协议映射（Anthropic `tool_use` <-> OpenAI `tool_calls`）。

### Milestone 4: Normalized Protocol 架构 ✅ 已完成

目标：

- 定义统一的标准化协议类型（`NormalizedRequest`、`NormalizedEvent`、`NormalizedMessage`）。
- 实现 `ProviderAdapter` trait，统一所有 provider 的接入方式。
- 实现 `OpenAiAdapter` 和 `AnthropicAdapter`，支持协议转换。
- 实现 tool use 跨协议映射（Anthropic `tool_use` <-> OpenAI `tool_calls`）。
- 实现 request ID 中间件，支持全链路追踪。
- 支持 stream fallback（first-chunk buffering）。
- 配置验证：circular fallback 检测、URL 验证、`first_token_timeout`。

已实现组件：

- `crates/ma-core/src/normalized.rs` — `NormalizedRequest`、`NormalizedEvent`、`NormalizedMessage`、`ToolDef`、`ToolChoice`、`ThinkingConfig`（568 行，23 个测试）
- `crates/ma-core/src/adapter.rs` — `ProviderAdapter` trait、`ProviderCapabilities`、`ProviderHealth`、`NormalizedResponse`、`StopReason`、`Usage`（7 个测试）
- `crates/ma-server/src/adapters/openai.rs` — `OpenAiAdapter`（857 行）
- `crates/ma-server/src/adapters/anthropic.rs` — `AnthropicAdapter`（887 行）
- `crates/ma-server/src/adapters/tool_mapping.rs` — Tool Use 跨协议映射（780 行，24 个测试）
- `crates/ma-server/src/adapters/mock.rs` — `MockAdapter`
- `crates/ma-server/src/middleware/request_id.rs` — Request ID 中间件
- `crates/ma-server/src/handlers/openai.rs` — 标准化 OpenAI 格式 handler
- `crates/ma-server/src/handlers/anthropic.rs` — 标准化 Anthropic 格式 handler
...
状态：Normalized Protocol 架构已完全实现。所有 provider 通过 `ProviderAdapter` trait 统一接入，支持 OpenAI 和 Anthropic 两种客户端协议的双向转换。69 个测试全部通过。

### Milestone 5: 可观测性和本地体验 🔄 部分完成
...
- `ma doctor` 命令已实现，支持配置验证、网络检查和 API key 检测。
- Request ID 中间件已实现，支持从请求头读取或自动生成 UUID，注入到所有日志和 tracing span 中。
- 配置验证已实现：circular fallback 检测、URL 格式验证、`first_token_timeout` 配置。

待完成：

- Token usage 统计和持久化。
- Provider latency 统计和持久化。
- 本地 dashboard（HTTP 管理接口）。
